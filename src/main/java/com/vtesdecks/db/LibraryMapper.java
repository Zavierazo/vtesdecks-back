...
public interface LibraryMapper {
    ...
    @Insert("INSERT INTO library (Id,Name,Aka,`Type`,Clan,Path,Discipline,Pool_Cost,Blood_Cost,Conviction_Cost,Burn,Text," +
            "Flavor,`Set`,Requirement,Banned,Artist,Capacity)"
            + "VALUES(#{Id}, #{Name}, #{Aka}, #{Type}, #{Clan}, #{Path}, #{Discipline}, #{PoolCost}, #{BloodCost}, " +
            "#{ConvictionCost}, #{Burn}, #{Text}, #{Flavor}, #{Set}, #{Requirement}, #{Banned}, #{Artist}, " +
            "#{Capacity})")
    void insert(DbLibrary entity);

    @Update("UPDATE library SET Name=#{Name}, Aka=#{Aka}, `Type`=#{Type}, Clan=#{Clan}, Path=#{Path}, Discipline=#{Discipline}, " +
