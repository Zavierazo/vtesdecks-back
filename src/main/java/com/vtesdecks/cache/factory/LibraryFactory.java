...
public abstract class LibraryFactory {
    ...
    protected void afterMapping(@MappingTarget Library library, LibraryEntity dbLibrary, @Context List<LibraryI18nEntity> libraryI18nList, @Context List<CardShopEntity> cardShopList, @Context List<LimitedFormatEntity> limitedFormats) {
        ...
        library.setTrifle(dbLibrary.getType().equals("Master") && dbLibrary.getText() != null && dbLibrary.getText().toLowerCase().contains("trifle."));
    ...
    private Set<String> getTypes(LibraryEntity library) {
    ...
    private Set<String> getTypeIcons(LibraryEntity library) {
        Set<String> types = new HashSet<>();
        List<String> cardTypes = new ArrayList<>();
        if (library.getType().contains("/")) {
            cardTypes.addAll(Splitter.on('/').trimResults().omitEmptyStrings().splitToList(library.getType()));
